...
subroutine do_analysis(dumpfile,numfile,xyzh,vxyz,pmass,npart,time,iunit)
 ...
 real :: temp(3),temp_mag,term(3),tilt_sinks,twist_sinks,L_tot_mag,h_vec(3) !PP
 ...
 if (nptmass>nptmassinit) then
    do i=nptmassinit+1,nptmass
       ...
       if (.not.iexist .or. numfile == 0) then
          ...
          write(isinks,"('#',15(1x,'[',i2.2,1x,a11,']',2x))") &
               1,'time', &
               2,'rad(sph)', &
               3,'Tilt sink', &
               4,'Twist sink', &
       ...
       ! Modification by P.Pajuelo (02/08/22)
       m_red = sinks_mass*xyzmh_ptmass(4,1)/(sinks_mass + xyzmh_ptmass(4,1))
       mu = 1.0*(sinks_mass + xyzmh_ptmass(4,1))  !mu = GM, G=1
       call cross_product3D(pos_sinks,vel_sinks,h_vec) !PP calculation of h
       print*,"Masse de l'étoile centrale =",xyzmh_ptmass(4,1) !PP Verification
       call cross_product3D(vel_sinks,h_vec,term) !PP New calculation
       ! call cross_product3D(vel_sinks,L_p/sinks_mass,term) !PP modification
       ...
       write(isinks,'(15(es18.10,1X))') time,break_disc_at, &
            tilt_sinks,twist_sinks,tilt_inner,tilt_outer, &
            twist_inner,twist_outer,L_ratio_inner,L_ratio_outer, &
            eff_tilt,eff_innertilt,eff_outertilt,e_sinks,rad_sinks 
